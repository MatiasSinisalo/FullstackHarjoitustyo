...
import courseService, {createCourse, addUserToCourse, removeUserFromCourse, getCourse,
  addTaskToCourse, addSubmissionToCourseTask, removeTaskFromCourse, removeSubmissionFromCourseTask} from '../services/courseService';
...
describe('courseService tests', () => {
  describe('createCourse function tests', () => {
    ...
    test('createCourse function returns correctly with incorrect parameters', async () => {
      mockClient.mutate = jest.fn(() => {
        throw new Error('this is some kind of an error');
      });
  ...
  describe('addTaskToCourse Tests', () => {
    test('addTaskToCourse calls backend with correct data and returns correctly', async () => {
      ...
      mockClient.mutate = jest.fn(() => {
        return {data: {addTaskToCourse: task}};
      });
      mockClient.readQuery = jest.fn(() => {
        return {getCourse: {uniqueName: 'test'}};
      });
      mockClient.cache.modify = jest.fn((x) => {
        return x;
      });
      mockClient.cache.identify = jest.fn();
  ...
  describe('addSubmissionToCourseTask Tests', () => {
    test('addSubmissionToCourseTask calls backend with correct data and returns correctly', async () => {
      ...
      mockClient.mutate = jest.fn(() => {
        return {data: {addSubmissionToCourseTask: submission}};
      });
      mockClient.readQuery = jest.fn(() => {
        return {getCourse: {uniqueName: 'test'}};
      });
  ...
  describe('removeTaskFromCourseTests', () => {
    test('removeTaskFromCourse calls backend with correct data', async () => {
      mockClient.mutate = jest.fn(() => {
        return {data: {removeTaskFromCourse: true}};
      });
      mockClient.cache.gc = jest.fn();
      mockClient.cache.evict = jest.fn((x) => {
        return x;
      });
      mockClient.cache.identify = jest.fn();
    ...
    test('removeTaskFromCourse returns error correctly', async () => {
      mockClient.mutate = jest.fn(() => {
        throw new Error('this is some kind of error');
      });
      const result = await removeTaskFromCourse('coursename', 'abc1234', mockClient);
  ...
  describe('removeSubmissionFromCourseTask tests', () => {
    test('removeSubmissionFromCourseTask calls backend correctly', async () => {
      mockClient.mutate = jest.fn(() => {
        return {data: {removeSubmissionFromCourseTask: true}};
      });
      mockClient.cache.gc = jest.fn();
      mockClient.cache.evict = jest.fn((x) => {
        return x;
