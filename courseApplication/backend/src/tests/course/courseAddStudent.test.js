...
describe('Course add student tests', () => {
  ...
  test('addStudentToCourse query adds reference to attendedCourses correctly', async () => {
    await helpers.logIn('username');
    await helpers.makeQuery({query: createCourse, variables: {uniqueName: 'course-owned-by-username', name: 'common name', teacher: 'username'}});
    await helpers.makeQuery({query: addStudentToCourse, variables: {addStudentToCourseUsername: 'students username', courseUniqueName: 'course-owned-by-username'}});
  ...
  test('addStudentToCourse query returns error given username not found if trying to add a student that does not exist, and does not modifyi database', async () => {
    ...
    await helpers.makeQuery({query: createCourse, variables: {uniqueName: 'course-owned-by-username', name: 'common name', teacher: 'username'}});
    const courseWithAddedStudent = await helpers.makeQuery({query: addStudentToCourse,
      variables: {addStudentToCourseUsername: 'this user does not exist', courseUniqueName: 'course-owned-by-username'}});
  ...
  test('addStudentToCourse query returns error if trying to add a student to a course that already exists in the course', async () => {
    ...
    await helpers.makeQuery({query: addStudentToCourse, variables: {addStudentToCourseUsername: 'students username', courseUniqueName: 'course-owned-by-username'}});
    const courseWithDoublicateAddedStudent = await helpers.makeQuery({query: addStudentToCourse,
      variables: {addStudentToCourseUsername: 'students username', courseUniqueName: 'course-owned-by-username'}});
