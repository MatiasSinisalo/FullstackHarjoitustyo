...
describe('course querying tests', () => {
  describe('getAllCourses query tests', () => {
    ...
    test('getAllCourses returns null task list', async () => {
      ...
      await helpers.makeQuery({query: createCourse, variables: {...courseData}});
      const createdTask = await helpers.makeQuery({query: addTaskToCourse,
        variables: {courseUniqueName: 'courses-name', description: 'this is a description for a task', deadline: new Date(Date.now()).toString()}});
